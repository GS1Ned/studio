  ...
  const renderOutput = (data: ConductIndependentResearchOutput) => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm bg-muted p-3 rounded-md whitespace-pre-wrap">{data.collectedInformation}</p>
      </div>
    ...
    return {
      ...
      modelEvaluationMetrics: { 
        "Coverage": (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild><span className="cursor-help underline decoration-dashed">{(Math.random() * 0.3 + 0.65).toFixed(2)}</span></TooltipTrigger>
              <TooltipContent><p>Estimated breadth of topics covered by the research.</p></TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ),
        "Relevance": (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild><span className="cursor-help underline decoration-dashed">{(Math.random() * 0.25 + 0.7).toFixed(2)}</span></TooltipTrigger>
              <TooltipContent><p>Estimated relevance of gathered information to the query.</p></TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )
      },
        ...
        <CardHeader>
          ...
          <CardDescription className="flex flex-col gap-2">
            ...
            <span className="text-xs text-muted-foreground flex items-center">
              ...
              Note: The web search functionality is currently simulated. Real web search integration is planned.
            </span>
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Image
            ...
            alt="AI conducting independent research"
            width={800}
            height={300}
            className="rounded-lg object-cover w-full"
            data-ai-hint="research discovery"
          />
        </CardContent>
      </Card>
