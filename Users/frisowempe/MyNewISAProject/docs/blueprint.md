...
*   **Implement ETLVRE Pipeline v1 & Live Vector Store:**
    ...
    *   **Tasks:**
        ...
        4.  **(Development Task):** Update the `queryVectorStoreTool` in `src/ai/tools/vector-store-tools.ts` to:
            *   Remove the `mockDocumentMetadataStore`.
            *   After retrieving neighbor `datapointId`s from the real Vertex AI Vector Search, implement logic to **fetch the corresponding full `DocumentChunk` data from Firestore** using these IDs.
...
*   **Documentation:** Thoroughly document ETLVRE v1, KG v1 schema, new Genkit tools for live data interaction, and required environment variables/IAM permissions for Vertex AI services and Firestore.
...
*   **Priorities:**
    1.  **Solidify Phase 1 Achievements:** Ensure all foundational strengthening tasks (App Hosting config, Firestore security, secrets, error handling, CI/CD outline) are robust and documented as complete. The `queryVectorStoreTool` is now refactored for Vertex AI (with metadata lookup still simulated), and `generateDocumentEmbeddings` uses `ai.embed()` and assigns `chunkId`.
