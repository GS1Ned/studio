...
*   **Code Structure & Schemas:** Zod schemas for AI flow inputs are centralized in `src/ai/schemas.ts`. `DocumentChunkSchema` is now a canonical definition. AI flows now check for null/undefined outputs from LLM prompts.
...
*   **Vector Store Query Tool (`vector-store-tools.ts`):** A conceptual Genkit tool (`queryVectorStoreTool`) simulating queries against a vector store, now enhanced to accept a mock query embedding and interact with an internal mock vector database.
...
*   **Conceptual Knowledge Graph Interaction Tool (New - Added Oct 26):**
    ...
    *   **Files Created/Modified:** `src/ai/tools/knowledge-graph-tools.ts`, `src/ai/tools/index.ts`, `docs/blueprint.md`.
...
*   **AI/ML Layer (Leveraging Vertex AI, orchestrated by Genkit):**
    ...
    *   Advanced Agent Development (Future): Vertex AI Agent Builder.
...
It has been updated to reflect the commencement of Phase 2 and the specific refactoring of the conceptual vector search flow, and the addition of the conceptual Knowledge Graph query tool.
